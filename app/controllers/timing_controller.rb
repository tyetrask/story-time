  ...
  def index
    respond_to do |format|
      ...
      format.json { 
        ...
        @pivotal_projects = pivotal_access.get_projects
        @pivotal_project = @pivotal_projects[1]
        @pivotal_iterations = pivotal_access.get_iterations(@pivotal_project['id'])
        @my_work = pivotal_access.get_my_work(@pivotal_project['id'])
        render json: {me: @me, pivotal_project: @pivotal_project, pivotal_projects: @pivotal_projects, pivotal_iterations: @pivotal_iterations, my_work: @my_work}
      }
    end
  end
